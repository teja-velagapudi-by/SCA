...
@Component
public class ForUpdateCheckRule implements RuleSet {
    @StaticAnalysisRule(id = "Sql check for For Update")
    public Issue execute(LocalSyntaxCommand command) {
        final Pattern pattern = Pattern.compile("(\\bfor update of\\b)");
        ...
        for (CommandUnit unit : units) {
            if (StringUtils.isNotBlank(unit.getSql())) {
                ...
                if(matcher.find()){
                    String description = "Please remove for update statement(s) from sql queries in the Command:  ";
