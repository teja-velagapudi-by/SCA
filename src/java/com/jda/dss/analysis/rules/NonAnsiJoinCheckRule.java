...
@Component
public class NonAnsiJoinCheckRule implements RuleSet {
    @StaticAnalysisRule(id = "Sql check for Non-ANSI Join")
    public Issue execute(LocalSyntaxCommand command) {
        final Pattern pattern = Pattern.compile("where[\\s]+[\\w]+\\.[\\w]+[\\s]*=[\\s]*[\\w]+\\.[\\w]+");
        ...
        for (CommandUnit unit : units) {
            if (StringUtils.isNotBlank(unit.getSql())) {
                ...
                if(matcher.find()){
                    String description = "Please use the SQL keyword JOIN to join tables in the Command:  ";
